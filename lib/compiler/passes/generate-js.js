...
function generateJS(ast, options) {
  ...
  function generateTables() {
    if (options.optimize === "size") {
      return [
        "var peg$literals = [",
        indent2(ast.literals.join(",\n")),
        "];",
        "var peg$regexps = [",
        indent2(ast.classes.join(",\n")),
        "];",
        "var peg$expectations = [",
        indent2(ast.expectations.join(",\n")),
        "];",
        "var peg$functions = [",
        indent2(ast.functions.join(",\n")),
        "];",
        "",
        "var peg$bytecode = [",
  ...
  function generateInterpreter() {
    ...
    parts.push([
      ...
      "            peg$fail(peg$expectations[bc[ip + 1]]);",
      "          }",
      "          ip += 2;",
      "          break;",
      "",
    ...
    function compile(bc) {
      ...
      while (ip < end) {
        switch (bc[ip]) {
          ...
          case op.MATCH_STRING:       // MATCH_STRING s, a, f, ...
            compileCondition(
              eval(ast.literals[bc[ip + 1]]).length > 1
                ? "input.substr(peg$currPos, "
                    + eval(ast.literals[bc[ip + 1]]).length
                    + ") === "
                    + l(bc[ip + 1])
                : "input.charCodeAt(peg$currPos) === "
                    + eval(ast.literals[bc[ip + 1]]).charCodeAt(0),
              1
            );
            break;
          ...
          case op.ACCEPT_STRING:      // ACCEPT_STRING s
            ...
            parts.push(
              eval(ast.literals[bc[ip + 1]]).length > 1
                ? "peg$currPos += " + eval(ast.literals[bc[ip + 1]]).length + ";"
                : "peg$currPos++;"
            );
            ip += 2;
            break;
