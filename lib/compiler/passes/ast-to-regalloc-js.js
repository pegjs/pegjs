...
function generateJavascript(ast, options) {
  ...
  var generate = visitor.build({
    rule: function(node, funcName, discard) {
      ...
      if (discard) {
        ruleIndexCode += ast.rules.length;
      }
      if (options.cache) {
        body.push(
          'var key = peg$currPos * ' + (2*ast.rules.length) + ' + ' + ruleIndexCode + ',',
      ...
      body.push(
        ...
        result.resolveBlock().join('\n'));

      if (options.cache) {
        body.push(['peg$resultsCache[key] = {nextPos: peg$currPos, ',
          'result: ', result.expression, '};'].join(''));
      }
      body.push(
  ...
  while (rulesToGenerate.length) {
    ...
    generated.push(generate(rule, ruleInfo.funcName, ruleInfo.discard));
  }
