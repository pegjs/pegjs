...
function generateBytecode(ast) {
  ...
  function addExpectedConst(value) {
    let index = expectations.indexOf(value);
  ...
  let generate = visitor.build({
    ...
    named(node, context) {
      let nameIndex = addExpectedConst(
    ...
    literal(node) {
      if (node.value.length > 0) {
        ...
        let expectedIndex = addExpectedConst(
    ...
    class(node) {
      ...
      let parts = "["
        ...
        + "]";
      let classIndex = addClassConst(regexp);
      let expectedIndex = addExpectedConst(
    ...
    any() {
      let expectedIndex = addExpectedConst("peg$anyExpectation()");
