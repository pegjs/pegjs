...
function generateBytecode(ast) {
  ...
  let generate = visitor.build({
    ...
    named(node, context) {
      let nameIndex = addConst(`peg$otherExpectation("${js.stringEscape(node.name)}")`);
    ...
    literal(node) {
      if (node.value.length > 0) {
        let stringIndex = addConst(`"${js.stringEscape(node.ignoreCase ? node.value.toLowerCase() : node.value)}"`);
        let expectedIndex = addConst(`peg$literalExpectation("${js.stringEscape(node.value)}", ${node.ignoreCase})`);
    ...
    class(node) {
      ...
      let expectedIndex = addConst(`peg$classExpectation(${parts}, ${node.inverted}, ${node.ignoreCase})`);
