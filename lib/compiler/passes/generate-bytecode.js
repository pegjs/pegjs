  ...
  var generate = visitor.build({
    ...
    rule: function(node) {
      node.bytecode = generate(node.expression, {
        ...
        rule:   node,  // rule node - owner of node tree
    ...
    rule_ref: function(node, context) {
      // If this is reference to template parameter.
      var paramRef = context.rule.params.indexOf(node.name);
      if (paramRef >= 0) {
        return [op.PARAM_REF, paramRef];
      }

      var argsBC = arrays.map(node.args, generate);
      var bcLengths = arrays.map(argsBC, function(a) { return a.length; });
      var result = [op.RULE, asts.indexOfRule(ast, node.name), argsBC.length];
      result = Array.prototype.concat.apply(result, bcLengths);
      result = Array.prototype.concat.apply(result, argsBC);
      return result;
    },
    
    template_arg: function(node, context) {
      return generate(node.expression, context);
    },
