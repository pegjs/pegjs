...
function generateJavascript(ast, options) {
  ...
  function generateInterpreter() {
    ...
    function generateCall() {
      ...
      return [
        ...
        '  bc[ip + 2],',
        '  peg$funcs[bc[ip + 1]].apply(null, params)',
        ');',
        '',
  ...
  function generateRuleFunction(rule) {
    ...
    function f(i) { return "peg$funcs[" + i + "]"; } // |actions[i]| of the abstract machine
  ...
  function generateSandbox() {
    var parts   = [],
        count   = 0,
        actions = ast.actions,
        rest    = ast.actions.slice(0);

    function doGenerateSandbox(initializer, namespace, i) {//TODO: May be share all API beetween all sandboxes?
      parts.push('  function sandbox' + i + '(input, options, parser, text, offset, line, column, expected, error) {');

      if (initializer) {
        parts.push(indent4("{" + initializer.code + "}"));
        parts.push('');
      }

      parts.push('    return {');
      arrays.each(actions, function(f, i) {
        if (f.namespace === namespace) {
          parts.push('      ' + i + ': function(' + f.params.join(', ') + ') {' + f.body + '},');
        }
      });
      parts.push('    };');
    ...
    function generate(initializer, namespace) {
      doGenerateSandbox(initializer, namespace, count++);
      rest = rest.filter(function(f) { return f.namespace !== namespace; });
    }

    parts.push('function peg$sandbox(input, options, parser, text, offset, line, column, expected, error) {');

    // First, generate all functions with corresponds initializers.
    if (ast.initializers) {
      arrays.each(ast.initializers, function(initializer) {
        generate(initializer, initializer.namespace);
      });
    }
    // Second, generate all functions without initializer.
    while (rest.length > 0) {
      generate(null, rest[0].namespace);
    }
    ...
    if (count > 1) {
      ...
      for (i = 0; i < count; ++i) {
        parts.push(
        '  combine(sandbox' + i + '(input, options, parser, text, offset, line, column, expected, error));'
        );
      }
      parts.push(
        '',
    ...
    if (count !== 0) {
      parts.push(
        '',
        '  return sandbox0(input, options, parser, text, offset, line, column, expected, error);'
      );
    }
    parts.push('}');
  ...
  parts.push(
    '    //} Rules',
    '',
    '    var peg$funcs = peg$sandbox(input, options, parser, text, offset, line, column, expected, error);'
  );
