  ...
  function generateRuleFunction(rule) {
    ...
    function compile(bc) {
      ...
      function compileRule() {// RULE r cnt (len bc)|cnt|
        var ruleName   = ast.rules[bc[ip + 1]].name,
            argCount   = bc[ip + 2],
            summaryLen = 0;
        ...
        for (var i = 0; i < argCount; ++i) {
          var len   = bc[ip + 3 + i];
          var begin = ip + 3 + argCount + summaryLen;

          parts.push(stack.push([
            'function() {',
            '//TODO: ',
            //compile(bc.slice(begin, begin + len));
            '}'
          ].join('\n')));
          summaryLen += len;
        }
    ...
    parts.push([
      'function peg$parse' + rule.name + '(' + arrays.map(rule.params, p).join(', ') + ') {',
