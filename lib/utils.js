var utils = {
  ...
  findAnnotationByName: function(rule, name) {
    return utils.find(rule.annotations, function(a) { return a.name === name; });
  },
  /// Finds all annotations with given name at given rule or empty array, if such annotations not exist.
  findAnnotationsByName: function(rule, name) {
    return utils.fiter(rule.annotations, function(a) { return a.name === name; });
  }
};
